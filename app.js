...
function mapRow(r, index) {
  ...
  const uniqueViewers = sourceYouTube ? number(lookup(r,['unique viewers'])) : Math.round(impressions / 2);
  const uniqueReach = sourceYouTube ? number(lookup(r,['unique reach'])) : 0;
  const spend = number(lookup(r,['cost','spend','cost (usd)','amount spent']));
  return { video, uniqueViewers, uniqueReach, views, engagement, impressions, spend };
}
function totals(rows) { return rows.reduce((a, r) => ({ uniqueViewers:a.uniqueViewers+r.uniqueViewers, uniqueReach:a.uniqueReach+r.uniqueReach, views:a.views+r.views, engagement:a.engagement+r.engagement, impressions:a.impressions+r.impressions, spend:a.spend+r.spend }), {uniqueViewers:0,uniqueReach:0,views:0,engagement:0,impressions:0,spend:0}); }
function render() {
  ...
  const reachLabel = currentSource === 'Google Ads' ? 'Est. reach' : 'Unique viewers';
  document.querySelector('#stats').innerHTML = [[reachLabel,fmt(total.uniqueViewers)],['Unique reach',total.uniqueReach ? fmt(total.uniqueReach) : '—'],['Views',fmt(total.views)],['Engagement',fmt(total.engagement)],['Spend',money(total.spend)]].map(([k,v]) => `<article class="stat"><p>${k}</p><strong>${v}</strong></article>`).join('');
  const cells = row => `<td>${row.video}</td><td>${fmt(row.uniqueViewers)}</td><td>${row.uniqueReach ? fmt(row.uniqueReach) : '—'}</td><td>${fmt(row.views)}</td><td>${fmt(row.engagement)}</td><td>${fmt(row.impressions)}</td><td>${money(row.spend)}</td>`;
  ...
  document.querySelector('#insight').innerHTML = `<strong>Quick read:</strong> ${total.views ? `${fmt(total.views)} views across ${outputRows.length} item${outputRows.length === 1 ? '' : 's'}.` : 'Your export did not include view values.'}${rate ? ` The observed view rate is ${rate}%.` : ''}${currentSource === 'Google Ads' ? ' Reach is estimated at half of impressions; Google Ads does not provide unique reach in the standard export.' : ' Unique viewers measures people who watched, while unique reach measures people shown the video.'}`;
  emptyState.classList.add('hidden'); results.classList.remove('hidden'); results.scrollIntoView({behavior:'smooth', block:'start'});
}
document.querySelector('#downloadButton').addEventListener('click', () => { const rows = [...outputRows, {video:'TOTAL',...totals(outputRows)}]; const text = ['Video,Unique viewers (Reach),Unique reach,Views,Engagement,Impressions,Spend', ...rows.map(r => `"${String(r.video).replaceAll('"','""')}",${r.uniqueViewers},${r.uniqueReach},${r.views},${r.engagement},${r.impressions},${r.spend.toFixed(2)}`)].join('\n'); const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([text],{type:'text/csv'})); a.download = 'campaign-summary.csv'; a.click(); URL.revokeObjectURL(a.href); });
document.querySelector('#copyButton').addEventListener('click', async e => { const rows = [...outputRows, {video:'TOTAL',...totals(outputRows)}]; await navigator.clipboard.writeText(['Video\tUnique viewers (Reach)\tUnique reach\tViews\tEngagement\tImpressions\tSpend', ...rows.map(r => `${r.video}\t${r.uniqueViewers}\t${r.uniqueReach}\t${r.views}\t${r.engagement}\t${r.impressions}\t${r.spend.toFixed(2)}`)].join('\n')); e.target.textContent = 'Copied'; setTimeout(() => e.target.textContent = 'Copy table', 1300); });
document.querySelector('#resetButton').addEventListener('click', () => { files=[]; outputRows=[]; currentSource=''; input.value=''; document.querySelector('#brandInput').value=''; document.querySelector('#marketInput').value=''; processButton.disabled=true; uploadStatus.textContent='No files selected'; results.classList.add('hidden'); emptyState.classList.remove('hidden'); document.dispatchEvent(new CustomEvent('workspace:reset')); window.scrollTo({top:0,behavior:'smooth'}); });
